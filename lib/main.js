...
function invertBy( obj, opts, transform ) {
	...
	var i;
	if ( !isObjectLike( obj ) ) {
		throw new TypeError( format( '2pPES', obj ) );
	}
	allowDupes = true;
	if ( arguments.length === 2 ) {
		...
		if ( !isObject( opts ) ) {
			throw new TypeError( format( '2pP2V', opts ) );
		}
		if ( hasOwnProp( opts, 'duplicates' ) ) {
			allowDupes = opts.duplicates;
			if ( !isBoolean( allowDupes ) ) {
				throw new TypeError( format( '2pP2o', 'duplicates', allowDupes ) );
			}
		}
		cb = transform;
	}
	if ( !isFunction( cb ) ) {
		throw new TypeError( format( '2pP3q', cb ) );
	}
	keys = objectKeys( obj );
	len = keys.length;
	out = {};
